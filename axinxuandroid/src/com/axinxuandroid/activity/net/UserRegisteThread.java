...
public class UserRegisteThread extends NetThread {

	private User user;
    private int registtype;
    private String oauthid;
	public UserRegisteThread(User user,int registtype,String oauthid) {
		this.user = user;
	    this.registtype=registtype;
	    this.oauthid=oauthid==null?"":oauthid;
	}
	...
	public void requestHttp() {
 		String url = HttpUrlConstant.URL_HEAD + HttpUrlConstant.USER_REGISTE;
		RequestParams params=new RequestParams();
 		if (user != null) {
 			params.put("phone", user.getPhone());
 			params.put("pwd", MD5.hexdigest(user.getPwd()));
 			params.put("registtype", registtype+"");
 			params.put("oauthid", oauthid);
  			if (user.getPerson_imageurl() != null) {
  				params.put("img", user.getPerson_imageurl());
 			}
			if (user.getUser_name() != null) {
				params.put("nick_name",   user.getUser_name() );
 			}
 		}
	...
	public NetResult onResponseSuccess(JSONObject jsonObject) {
		try {
			if (jsonObject != null&& jsonObject.getInt("ajax_optinfo") ==NET_RETURN_SUCCESS&&jsonObject.getInt("hasdata") == HAS_DATA) {
				...
				// ×¢²áÊ§°Ü
				return new NetResult(NetResult.RESULT_OF_ERROR, "×¢²áÊ§°Ü£¡");
			}
		}   catch (Exception e) {
 			e.printStackTrace();
