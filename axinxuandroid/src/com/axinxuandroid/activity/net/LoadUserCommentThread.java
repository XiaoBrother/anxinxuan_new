...
public class LoadUserCommentThread extends NetThread {

	private int userid;
	private int type;
	private String lastoptime;
	public LoadUserCommentThread(int userid,int type, String lastoptime){
		this.userid=userid;
		this.type=type;
		...
		try {
			if (jsonObject.getInt("hasdata") == HAS_DATA&&jsonObject.has("comments")) {
				...
				for (int i = 0; i < array.length(); i++) {
					...
					comment.setRecordid(jo.getInt("record_id"));
